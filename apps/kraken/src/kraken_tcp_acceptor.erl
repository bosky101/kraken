...
init(TcpServer, ListenSocket, Module) ->
  case gen_tcp:accept(ListenSocket) of
    ...
    {error, closed} ->
      lager:warning("Connection closed while accepting connection in tcp acceptor"),
      exit(normal);
    {error, timeout} ->
      lager:warning("Timeout reached in tcp acceptor"),
      exit(normal);
    Other ->
      error_logger:error_report(
        [{application, kraken},
...
relay(Module, Socket, State) ->
  case gen_tcp:recv(Socket, 0, ?IDLE_TIMEOUT_MS) of
    ...
    {error, closed} ->
      lager:warning("Connection closed while receiving data in tcp acceptor"),
      gen_tcp:close(Socket);
    {error, timeout} ->
      Module:handle_client_timeout(Socket, State),
      lager:error("Client timed out after ~p milliseconds", [?IDLE_TIMEOUT_MS]),
      gen_tcp:close(Socket);
    Other ->
